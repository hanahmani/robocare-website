...
export function Breadcrumbs({ items }: { items: readonly Crumb[] }) {
  ...
  return (
    ...
    >
      <Link href="/" className="text-white/50 transition-colors hover:text-lime-500">
        {t('nav.home')}
      </Link>
      {items.map((item) => (
        <span key={item.labelKey} className="flex items-center gap-2.5">
          ...
          {item.href ? (
            <Link href={item.href} className="text-white/50 transition-colors hover:text-lime-500">
              {t(item.labelKey)}
            </Link>
          ) : (

...
export function ApproachCard({ icon: Icon, tone, title, text }: Props) {
  ...
  return (
    <article
      className={cn(
        'flex h-full flex-col rounded-card border border-forest-950/[0.08] bg-white p-7 shadow-soft transition-surface duration-slow ease-premium hover:-translate-y-1.5 hover:shadow-hover motion-reduce:hover:translate-y-0 lg:p-8',
        palette.border,
      )}
    >
      <div className="flex items-center gap-4">
        <IconChip tone={tone} className="h-11 w-11 shrink-0 rounded-field">
